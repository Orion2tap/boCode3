    ...
    <div id='content'>
        <div class='panel'>
            ...
            <div class='inner post'>
                <form id='create_topic_form' action='/topic/edit/${topic.id}' method='post'>
                    <fieldset>
                        <span class="tab-selector">选择版块：</span>
                        <select name="boardId" id="tab-value" data-board-id=${topic.board.id}>
                            <option value="">请选择</option>
<!--                            <option value="share" selected>分享</option>-->
                            <#list boards as b>
                            <option class="option-value" value="${b.id}" >${b.name}</option>
                            </#list>
                        </select>
                        ...
                        <div class='markdown_editor in_editor'>
                            <div class='markdown_in_editor'>
                                <textarea class='editor' name='content' rows='20' placeholder='文章支持 Markdown 语法, 请注意标记代码' maxlength="1000">${topic.content}</textarea>
                                <!-- CSRF Token -->
                        ...
                        <input type='hidden' name='_csrf' value='YoFjGLUV-n5PB5lLd0l1_kG4XKhIMYm-K4KE'>
                    </fieldset>
                </form>
            </div>
        </div>
